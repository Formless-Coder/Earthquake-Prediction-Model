...
export const MetricsPanel: React.FC<MetricsPanelProps> = ({ prediction }) => {
  return (
    <div className="p-8 h-full flex flex-col justify-between space-y-6">
      <div className="elevated-panel p-10 flex-1 flex flex-col justify-center space-y-6 bg-seismic-surface-1/40 border-white/5 shadow-2xl">
        <div className="flex justify-between items-start">
          ...
          <div className={`px-4 py-1.5 text-[10px] font-sans font-black tracking-widest uppercase rounded-full border ${
            (prediction?.magnitude || 0) > 6 
            ? 'bg-seismic-danger/10 text-seismic-danger border-seismic-danger/20' 
            : 'bg-seismic-cyan/10 text-seismic-cyan border-seismic-cyan/20'
          }`}>
            {prediction ? (prediction.magnitude > 6 ? 'CRITICAL' : 'STABLE') : 'WAITING'}
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        <div className="elevated-panel p-6 space-y-2 bg-seismic-surface-1/30">
          ...
          <div className="flex items-baseline gap-2">
            <span className="font-data text-3xl font-bold text-white">{prediction ? prediction.depth : '--.-'}</span>
        ...
        <div className="elevated-panel p-6 space-y-2 bg-seismic-surface-1/30">
          ...
          <div className="flex items-baseline gap-2">
            <span className="font-data text-3xl font-bold text-white">{prediction ? prediction.radius : '--.-'}</span>
            <span className="seismic-label text-seismic-cyan/60 font-black">KM</span>
          </div>
        </div>
      </div>
